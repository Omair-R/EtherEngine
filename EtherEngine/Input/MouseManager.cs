...
namespace EtherEngine.Input
{
    ...
    public sealed class MouseManager : LazySingleton<MouseManager>
    {
        ...
        {
            return GetButtonState(button, this._mouseState) == ButtonState.Pressed;
        }

        public bool IsClicked(MouseButtons button)
        {
            return GetButtonState(button, this._mouseState) == ButtonState.Pressed &&
                !(GetButtonState(button, this._prevMouseState) == ButtonState.Pressed);
        }

        public bool IsReleased(MouseButtons button)
        {
            return GetButtonState(button, this._prevMouseState) == ButtonState.Pressed &&
                !(GetButtonState(button, this._mouseState) == ButtonState.Pressed);
        }
